        ...
        {loading ? (
          ...
          <div key={item.id} className="bg-card-dark border border-slate-800 p-4 rounded-2xl flex items-center justify-between group hover:border-primary/30 transition-all active:scale-[0.98]">
            ...
            <div className="flex flex-col items-end gap-3">
              ...
              <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-md ${item.stock === 'high' ? 'bg-green-500/10' : 'bg-red-500/10'}`}>
                <span className={`text-[9px] font-black uppercase ${item.stock === 'high' ? 'text-green-500' : 'text-red-500'}`}>
                  STOCK: {item.stockVal}
                </span>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-card-dark rounded-3xl p-8 border border-white/10 shadow-2xl">
            ...
            <form onSubmit={guardarCambios} className="space-y-4">
              ...
              <button className="w-full py-5 bg-primary hover:bg-primary/90 rounded-2xl font-black uppercase text-sm tracking-widest transition-all active:scale-95 flex items-center justify-center gap-3">
                {isSaving ? <div className="size-5 border-2 border-white/20 border-t-white rounded-full animate-spin"></div> : 'GUARDAR CAMBIOS'}
              </button>
            </form>
          </div>
        </div>
